...
int zgt_tx::set_lock(long tid1, long sgno1, long obno1, int count, char lockmode1){
  ...
  //write your code
  zgt_tx *existingtx = get_tx(tid1);
  // Lock the transation manager
  zgt_p(0);
  // Checking if the transaction is present or not
  zgt_hlink *currenttx = ZGT_Ht->find(sgno1, obno1);
  // unlock the transaction manager
  zgt_v(0);

  // If there is no transaction then add
  if(currenttx == NULL)
  {
    // Lock the Transaction Manager
    zgt_p(0);
    // Add transaction
    ZGT_Ht->add(existingtx,sgno1, obno1, lockmode1);
    zgt_v(0);
    // Now performing read write operations on exisiting transaction
    existingtx->perform_readWrite(tid1, obno1, lockmode1);
  }
  
  
  // If the current transation and existing transaction are same
  else if(existingtx->tid == currenttx->tid)
  {
    existingtx->perform_readWrite(tid1, obno1, lockmode1);
  }
  ...
  {
    zgt_p(0);
    // Find the transaction which has the lock from the hash table
    zgt_hlink *findtx = ZGT_Ht->findt(tid1, sgno1, obno1);
    zgt_v(0);
    ...
    else if(lockmode1 == 'X' || (lockmode1 == 'S' && currenttx-> lockmode == 'X') || (lockmode1 == 'S' && currenttx->lockmode == 'S' && zgt_nwait(currenttx->tid) > 0))
    {
        existingtx->obno = obno1;
        existingtx->lockmode = lockmode1;
        existingtx->status = TR_WAIT;
        //Semaphore to a transcation.
        existingtx->setTx_semno(currenttx->tid,currenttx->tid);
        // printf("The transaction %d is waiting at transaction %d for an object number %d", tid1, currenttx->tid, obno1);
        // fflush(stdout);
        // Lock the current transaction
        zgt_p(currenttx->tid);

        existingtx->obno = -1; // resetting the lock of transaction, indicates that it does not hold ay lock on any particular object
        existingtx->lockmode = ' '; // resetting the lock of transaction, indicates that it does not hold any lock on any particular object
        existingtx->status = TR_ACTIVE; // transaction is active and can proceed with read/write operations
        // printf("The transaction %d is waiting at transaction %d for object number %d", tid1, currenttx->tid, obno1);
        // fflush(stdout);

        //Performing Read Write on that transaction
        existingtx->perform_readWrite(tid1, obno1, lockmode1);
        // Releasing current transaction
        zgt_v(currenttx->tid);
    }
...
zgt_hlink *zgt_tx::others_lock(zgt_hlink *h, long sgno, long obno) 
{
  while (h != NULL)
  {
    if (h->sgno == sgno && h->obno == obno && h->tid != tid)
    {
      return h;
    }
    h = h->nextp;
...
void zgt_tx::perform_readWrite(long tid,long obno, char lockmode){
  ...
  {
    printf("\n Error no lockmode for transaction \n");
    zgt_v(0);
  }
